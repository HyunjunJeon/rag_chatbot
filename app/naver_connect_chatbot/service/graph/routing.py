...
def check_answer_quality(
    state: AdaptiveRAGState
) -> Literal["finalize", "correct", "return_best_effort"]:
    """
    답변 품질을 확인하고 다음 단계를 결정합니다.
    ...
    max_corrections = 2  # 교정 반복 허용 횟수
    
    logger.debug(
        f"Checking answer quality: score={quality_score:.2f}, "
        f"hallucination={has_hallucination}, complete={is_complete}, "
        f"corrections={correction_count}"
    )
    
    # 교정 허용 횟수를 초과했는지 확인합니다.
    if correction_count >= max_corrections:
        logger.warning(
            f"Max correction iterations ({max_corrections}) exceeded, "
            "returning best available answer"
        )
        return "return_best_effort"
    
    # 답변이 모든 품질 검사를 통과했는지 확인합니다.
    if quality_score >= 0.8 and not has_hallucination and is_complete:
        logger.info("Answer passes quality checks, finalizing")
        return "finalize"
    
    # 답변 교정이 필요합니다.
    logger.info(
        f"Answer needs correction (score={quality_score:.2f}, "
        f"hallucination={has_hallucination}, complete={is_complete})"
    )
    return "correct"


def should_refine_or_regenerate(
    state: AdaptiveRAGState
) -> Literal["analyze_query", "generate_answer"]:
    """
    질의를 다시 정제할지, 답변을 재생성할지 결정합니다.
    
    교정 에이전트가 추천한 액션에 따라 분기합니다.
    ...
    반환값:
        다음 노드 이름
    """
    correction_action = state.get("correction_action", "REGENERATE")
    
    logger.debug(f"Correction action: {correction_action}")
    
    if correction_action in ["REFINE_QUERY", "ADD_CONTEXT"]:
        logger.info("Refining query for better retrieval")
        return "analyze_query"
    else:  # REGENERATE, CLARIFY, or default
        logger.info("Regenerating answer with current context")
        return "generate_answer"


def route_after_correction(
    state: AdaptiveRAGState
) -> Literal["analyze_query", "generate_answer", "finalize"]:
    """
    교정 전략 결정 이후의 흐름을 정의합니다.
