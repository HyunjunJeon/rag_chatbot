...
class DocumentScore(BaseModel):
    ...
    evidence: str = Field(description="Decisive evidence snippet supporting the score", default="")
...
class DocumentEvaluation(BaseModel):
    ...
    relevant_count: int = Field(description="Number of relevant documents", ge=0)
    irrelevant_count: int = Field(description="Number of irrelevant documents", ge=0)
    sufficient: bool = Field(description="Whether documents are sufficient to answer the question")
    confidence: float = Field(
        description="Confidence in the evaluation (0.0 ~ 1.0)", ge=0.0, le=1.0
    )
    improvement_suggestions: list[str] = Field(
        description="Suggestions for improving retrieval", default_factory=list
    )
...
def emit_document_evaluation(
    ...
    sufficient: Annotated[
        bool, Field(description="Whether documents are sufficient to answer the question")
    ],
    confidence: Annotated[
        float, Field(description="Confidence in the evaluation (0.0 ~ 1.0)", ge=0.0, le=1.0)
    ],
    improvement_suggestions: Annotated[
        list[str], Field(description="Suggestions for improving retrieval")
    ],
    doc_scores: Annotated[
        list[DocumentScore], Field(description="Per-document scores and evidence")
    ],
) -> DocumentEvaluation:
    """
...
def create_document_evaluator(llm: Runnable) -> Any:
    ...
    try:
        ...
        logger.debug(
            "Document evaluator agent created successfully with emit_document_evaluation tool"
        )
        return agent
...
def evaluate_documents(
    question: str, documents: list[Document], llm: Runnable
) -> DocumentEvaluation:
    """
