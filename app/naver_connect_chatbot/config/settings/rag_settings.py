...
class AdaptiveRAGSettings(BaseSettings):
    ...
    enable_intent_classification: bool = Field(
        default=True, description="Enable intent classification for adaptive strategies"
    )
    ...
    max_retrieval_retries: int = Field(
        default=2, ge=0, le=5, description="Maximum number of retrieval retry attempts"
    )
    max_correction_retries: int = Field(
        default=2, ge=0, le=5, description="Maximum number of correction iteration attempts"
    )
    ...
    agent_timeout: int = Field(
        default=30, ge=10, le=300, description="Timeout for agent operations in seconds"
    )
    retrieval_timeout: int = Field(
        default=10, ge=5, le=60, description="Timeout for retrieval operations in seconds"
    )
