...
class RetrieverSettings(BaseSettings):
    ...
    default_k: int = Field(default=10, ge=1, le=100, description="검색 시 반환할 문서 수")
    default_sparse_weight: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Sparse 검색기 가중치 (0.0 ~ 1.0)"
    )
    default_dense_weight: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Dense 검색기 가중치 (0.0 ~ 1.0)"
    )
    default_rrf_c: int = Field(default=60, ge=1, description="RRF(Reciprocal Rank Fusion) 상수")
...
class MultiQuerySettings(BaseSettings):
    ...
    rrf_k: int = Field(default=60, ge=1, description="MultiQuery RRF 상수")
    include_original: bool = Field(
        default=True, description="원본 쿼리를 생성된 쿼리 목록에 포함할지 여부"
    )
...
class AdvancedHybridSettings(BaseSettings):
    ...
    base_hybrid_weight: float = Field(
        default=0.4, ge=0.0, le=1.0, description="Base Hybrid 가중치 (Final Hybrid 구성 시)"
    )
    multi_query_weight: float = Field(
        default=0.6, ge=0.0, le=1.0, description="MultiQuery 가중치 (Final Hybrid 구성 시)"
    )
