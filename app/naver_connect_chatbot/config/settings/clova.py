...
class ClovaXLLMSettings(BaseSettings):
    ...
    model: str = Field(default="HCX-007", description="사용할 Clova X 모델명 (HCX-003, HCX-007 등)")
    temperature: float = Field(
        default=0.7, ge=0.0, le=1.0, description="응답의 창의성 조절 (0.0: 결정적, 1.0: 창의적)"
    )
    max_tokens: int | None = Field(
        default=None, description="생성할 최대 토큰 수 (None이면 제한 없음)"
    )
    thinking_effort: str | None = Field(
        default=None, description="Thinking 모드 effort: 'none', 'low', 'medium', 'high' 또는 None"
    )
    ...
    def validate_thinking_effort(cls, v: str | None) -> str | None:
        ...
        if v.lower() not in valid_values:
            raise ValueError(f"thinking_effort는 {valid_values} 중 하나여야 합니다. 입력값: {v}")
        if v is None:
            return "none"
        return v.lower()
...
class ClovaXEmbeddingsSettings(BaseSettings):
    ...
    embeddings_model: str = Field(
        default="bge-m3", description="임베딩 모델명 (clir-emb-dolphin, bge-m3 등)"
    )
...
class ClovaStudioRerankerSettings(BaseSettings):
    ...
    max_tokens: int = Field(
        default=4096, ge=1, le=4096, description="최대 생성 토큰 수 (기본값: 4096, 최대: 4096)"
    )
    request_timeout: float = Field(
        default=60.0, ge=1.0, le=300.0, description="HTTP 요청 타임아웃 (초) 기본 60초, 최대 300초"
    )
...
class ClovaStudioSegmentationSettings(BaseSettings):
    ...
    post_process_max_size: int = Field(
        default=1000, ge=1, description="후처리 적용 시 문단에 포함되는 문자열의 최대 글자 수"
    )
    ...
    request_timeout: float = Field(
        default=60.0, ge=1.0, le=300.0, description="HTTP 요청 타임아웃 (초)"
    )
...
class ClovaStudioSummarizationSettings(BaseSettings):
    ...
    auto_sentence_splitter: bool = Field(
        default=True, description="요약할 문장 목록의 문장 분리 허용 여부"
    )
    seg_count: int = Field(
        default=-1,
        ge=-1,
    ...
    include_ai_filters: bool = Field(default=False, description="AI Filter 적용 여부")
    request_timeout: float = Field(
        default=60.0, ge=1.0, le=300.0, description="HTTP 요청 타임아웃 (초)"
    )
...
class ClovaStudioRAGReasoningSettings(BaseSettings):
    ...
    stop: list[str] = Field(
        default_factory=list, description="토큰 생성 중단 문자 리스트 (기본값: [])"
    )
    seed: int = Field(
        default=0,
        ge=0,
    ...
    include_ai_filters: bool = Field(
        default=True, description="AI 필터 결과 표시 여부 (욕설, 비하/차별/혐오, 성희롱/음란 등)"
    )
    request_timeout: float = Field(
        default=60.0, ge=1.0, le=300.0, description="HTTP 요청 타임아웃 (초)"
    )
