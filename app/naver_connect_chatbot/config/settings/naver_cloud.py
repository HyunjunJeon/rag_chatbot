...
class ClovaStudioRerankerSettings(BaseSettings):
    ...
    endpoint: str | None = Field(default=None, description="Reranker API 엔드포인트 URL")
    max_tokens: int = Field(
        default=1024, ge=1, le=4096, description="최대 생성 토큰 수 (기본값: 1024, 최대: 4096)"
    )
    request_timeout: float = Field(
        default=30.0, ge=1.0, le=300.0, description="HTTP 요청 타임아웃 (초)"
    )
