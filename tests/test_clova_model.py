...
if not api_key:
    ...
    def run_case(name: str, llm: ChatClovaX) -> None:
        print(f"\n[Case] {name}")
        try:
            ...
            print(f"  호출 실패: {e}")
            # 내부 HTTP 에러가 노출되는 경우 상태코드/본문도 함께 출력
    ...
    # Case 1: 기본 호출 (tools/reasoning 없음)
    # llm_base = ChatClovaX(model="HCX-007")
    # run_case("base (no tools, no reasoning)", llm_base)

    # Case 2: reasoning_effort 만 설정
    # llm_reasoning = ChatClovaX(
    #     model="HCX-007",
    #     reasoning_effort="low",
    # )
    # run_case("reasoning_effort only", llm_reasoning)
    ...
    # Case 4: tools 만 설정 (reasoning 설정)
    """
    [Case] tools only (bind_tools)
  호출 실패: Error code: 400 - {'error': {'message': 'Invalid parameter: tools, reasoning. Please refer to API documentation ensure all parameter values, types, and formats are correct', 'code': '40001'}}
  HTTP 400
  응답 본문 (앞 500자): {"error":{"message":"Invalid parameter: tools, reasoning. Please refer to API documentation ensure all parameter values, types, and formats are correct","code":"40001"}}
    """
    llm_tools = ChatClovaX(
        model="HCX-007",
        max_tokens=20480,
